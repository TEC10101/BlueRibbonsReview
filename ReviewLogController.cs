...
namespace Blue_Ribbon.Controllers
{
    ...
    public class ReviewLogController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            //var Logs1 = db.ReviewLog.Select(x => x.ASIN).ToArray();
            //List<Deal> DealsFast = db.Deal.Where(x => Logs1.Contains(x.ASIN)).ToList();

            var data = from DealRecord in db.Deal
                       join ReviewRecord in db.ReviewLog on DealRecord.ASIN equals ReviewRecord.ASIN
                       select new { ReviewRecord, DealRecord.Name };

            var LogsViewModel = new List<ReviewLogViewModel>();

            foreach (var item in data)
            {
                var vm = new ReviewLogViewModel(new Deal { Name = item.Name }, item.ReviewRecord);
        ...
        public ActionResult Create(ReviewLog Review)
        {
        ...
        public ActionResult CreateMock([Bind(Include = "ReviewLogId,ASIN,WebsiteAPIId,CustomerReviewed,AutomaticValidation,AdminReviewed,DisplayReview,Rating,Email,ReviewSubject,ReviewBody,WouldBuyAgain,RecToFriend")] ReviewLog reviewLog)
        {
            ...
            //A request goes into our campaign dB to pull a code out
            //The page changes to display message "thank you, here is your code"
            //insert code on screen
            //Add dB record with date, productid, campaignid
